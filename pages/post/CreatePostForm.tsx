...
export default function CreatePostForm() {
    ...
    useEffect(() => { handleBtn(select, title, description) }, [select, title, description])

    const handleBtn = (select: boolean, title: string, description: string) => (
        !((title.length > 0 && title.length <= 50) && (description.length >= 30 && description.length <= 1000) && select)
    )
    ...
    return (
        <div className="flex flex-col gap-20 w-[100%]">
            ...
            <div>
                <Checkbox defaultSelected={select} onChange={e => setSelectValue(e) } css={{ fontSize: '10px'}}>
                ...
                <Button
                    ...
                    className={`${ handleBtn(select, title, description) ? "bg-[#E5E5E5] text-[#ABABAB]" : "bg-primaryPink" } text-white hover:bg-secondaryPink hover:text-primaryPink active:bg-[#ffb8b8] active:text-white focus:bg-primaryPink`}
                >
                        Dodaj ogłoszenie
                </Button>
            </div>
